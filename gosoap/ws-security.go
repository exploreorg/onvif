...
func NewSecurity(username, passwd string, timeOffset int) Security {
	...
	now := time.Now().UTC()
	deviceTime := now.Add(time.Duration(timeOffset) * time.Second)
	created := deviceTime.Format(time.RFC3339Nano)
	auth := Security{
		Auth: wsAuth{
			Username: username,
			Password: password{
